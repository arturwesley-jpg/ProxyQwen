#!/usr/bin/env python3
"""
Teste simples com debug completo
"""
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=180)) as resp:
                ...
                if resp.status == 200:
                    ...
                    return Result(account["email"], False, elapsed, resp.status, "", body[:300])
...
async def main():
    ...
    for i, (account, message) in enumerate(zip(ACCOUNTS, TEST_MESSAGES)):
        print(f"\n[{i+1}/2] {account['email']}: {message[:40]}...")
        result = await send_request(account, message, i)
        
        if result.success:
            print(f"  ✓ Sucesso: {result.response_time_ms:.0f}ms, {result.content[:120]}...")
        else:
            print(f"  ✗ Falha ({result.status_code}): {result.error[:200]}")
